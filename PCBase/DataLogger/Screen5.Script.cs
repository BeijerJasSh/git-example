...
namespace Neo.ApplicationFramework.Generated
{
    ...
    public partial class Screen5
    {
		...
		void Screen5_Opened(System.Object sender, System.EventArgs e)
		{
			...
			{
				case 1:
					folderpath = Path.Combine(Environment.CurrentDirectory, "Project Files",
						ApplicationConstants.DatabaseExportFolder, "Data Loggers");
					break;
				case 2:
					...
					break;
				case 3:
					folderpath = Globals.Tags.ExportPath.Value.ToString().TrimEnd('\\');
					break;
			}
			// ***
			try
		...
		void SetImportDictionary(){
			string keyName1 = Globals.ScriptModule1._importDictionary.ElementAt(0).Key;
			string keyName2 = Globals.ScriptModule1._importDictionary.ElementAt(1).Key;
			Globals.ScriptModule1._importDictionary[keyName1] = dt.Columns[2].ColumnName.ToString();
			Globals.Tags.ImportColumnName01.Value = dt.Columns[2].ColumnName.ToString();
			Globals.ScriptModule1._importDictionary[keyName2] = dt.Columns[3].ColumnName.ToString();
		...
		void btnImport_Click(System.Object sender, System.EventArgs e)
		{
			...
			try{
				...
				var importKey = Globals.ScriptModule1._importDictionary.FirstOrDefault(x => x.Value == dt.Columns[2].ColumnName).Key;
				var importKey1 = Globals.ScriptModule1._importDictionary.FirstOrDefault(x => x.Value == dt.Columns[3].ColumnName).Key;
				// ***
				...
				for (int j = 0; j <dt.Rows.Count; j++){
					...
					sb.AppendLine(");");
					Globals.ScriptModule1.SQLiteInsertUpdateDelete(sb.ToString());
				}
			}
			catch(Exception ex){
				MessageBox.Show(ex.ToString());
		...
		{
			string sql="";
			sql="DELETE FROM DataLogger2 ";
			Globals.ScriptModule1.SQLiteInsertUpdateDelete(sql);
		}
    }
}
